...
- CI runs as parallel jobs: static gates once, the tests on Node 22, 24, and 26 on Linux plus macOS, a smoke test of the README install on both, and a CodeQL workflow. A newer push to a pull request cancels the older run.
...
- `pnpm install` sets up a git pre-commit hook that runs `pnpm check`, so a stale `dist/` is caught before the commit instead of in CI. CI has a `ci-ok` job that passes only when every other job did: the one check to require on `main`.
