...
- `verify`, `ignore`, and `allow` in a `.canny.json` are ignored until the file is trusted, since it sits in the repository the agent is editing: a cloned repo can ship one and a blocked agent can write one. `rules` and `strict` are still read from any config. Existing projects need one `canny trust` for those three fields to work again.
