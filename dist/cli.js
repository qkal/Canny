...
switch (cmd) {
    ...
    case "sessions":
        for (const s of listSessions())
            console.log(`${new Date(s.mtime).toISOString()}  ${s.file}  ${sessionCwd(read(s.file)) ?? "(project not recorded)"}`);
        break;
    case "replay":
        replay();
        break;
...
function pick() {
    if (target)
        return { file: target, entries: read(target) };
    const found = latestSession(process.cwd());
    if (!found)
        console.log(`no sessions recorded for ${process.cwd()} under ${home()}; \`${self()} sessions\` lists every project's`);
    return found;
}
function status() {
