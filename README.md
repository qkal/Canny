...
| A check piped into `tail`, such as `npm test 2>&1 \| tail -20`, which hides the check's exit status                                                                                                                                                                                    | PreToolUse  | pattern    | rewritten to run behind `set -o pipefail`      |
...
The rewrite keeps the output the agent asked for and makes the check's own exit status the command's, so a failing suite piped into `tail` is reported as failing and a passing one counts. Only `tail` is rewritten: `head` and `grep -q` stop reading early and would fail a passing check. On Claude Code the rewritten command still goes through the user's permission rules; Codex takes a rewrite only with an "allow" and asks for approval in a separate step.
