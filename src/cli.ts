...
function status(): void {
  ...
  console.log(
    `stops     ${stops.map((e) => (e.type === "verdict" ? e.decision : "")).join(", ") || "none yet"}`,
  );
  console.log(
    `jev       ${jev.length} calls, ${jev.filter((e) => e.type === "jev" && e.cached).length} cached, ${jev.filter((e) => e.type === "jev" && e.error).length} failed`,
  );
}
