...
export function loadRules(cwd: string, config: Config): RuleSet | null {
...
export function extractRules(markdown: string): string[] {
  const out: string[] = [];
  let inFence = false;
  let pending = "";
  const flush = (): void => {
    const t = pending.trim();
    if (t.length >= 12 && (STRONG.test(t) || SOFT.test(t))) out.push(t);
    pending = "";
  };
